...
ChangeMonitor.prototype.monitor = function(target, opts, callback) {
    var monitor = this;
    
    // if we have a client then send through the monitor command
    if (this.client) {
        
    }
    // otherwise, create the appropriate monitor locally
    else {
        monitor.attachMonitor(target, opts, callback);
    }
};
...
module.exports = function(target, opts, callback) {
    ...
    _monitor.monitor(target, opts, callback);
};
