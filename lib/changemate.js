...
function _setupReconnector(initialErr, notifier, opts) {
    ...
    if (opts.reconnectDelay) {
        ...
        // if we hit an initial error, then schedule a reconnect right now
        if (initialErr) {
            setTimeout(reconnect, opts.reconnectDelay);
        }
    }
    ...
    if (lib && lib.monitor) {
        ...
        lib.monitor(this, target, opts, function(err, notifier) {
            ...
            if (notifier.reconnect) {
                _setupReconnector(err, notifier, opts);
            }

            // emit the connect event (on next tick so it can be capture in the callback)
