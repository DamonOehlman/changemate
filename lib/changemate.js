 ...
 */
function _guessType(target, opts) {
    ...
    opts.type = _aliases[opts.type] ||  opts.type;
...
ChangeMonitor.prototype.createNotifier = function(type, target, opts, checkpoint) {
    var lib, notifier;
    ...
    if (lib && lib.Notifier) {
        ...
        // create the notifier
        notifier = new lib.Notifier(target, opts, checkpoint || {});
        notifier.type = type;
...
ChangeMonitor.prototype.watch = function(target, opts, checkpoint) {
    // ensure we have valid opts
    opts = opts || {};
    
    // guess the notifier type
    target = _guessType(target, opts);
    
    // initialise the reconnect delay
    opts.reconnectDelay = typeof opts.reconnectDelay != 'undefined' ? opts.reconnectDelay : 5000;
    
    return this.createNotifier(opts.type, target, opts, checkpoint);
};

// create the monitor
_monitor = exports = module.exports = new ChangeMonitor();

// expose the aliases allowing user modification
